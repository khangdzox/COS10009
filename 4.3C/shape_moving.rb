...
class GameWindow < Gosu::Window
  ...
  def update
    ...
    if button_down?(Gosu::KbRight)
      # if not close to the border, move freely
      if @shape_x < WIDTH - SHAPE_DIM - 3
        @shape_x += 3
      # if too close or outside the border, move to the border
      else
        @shape_x = WIDTH - SHAPE_DIM
      end
    end
    ...
    if button_down?(Gosu::KbDown) 
      if @shape_y < HEIGHT - SHAPE_DIM - 3
        @shape_y += 3
      else
        @shape_y = HEIGHT - SHAPE_DIM
      end
    end
