...
class Cell
  ...
  def get_neighbour # Get method - by me
    "north:#{@north != nil ? "1" : "0"} south: #{@south != nil ? "1" : "0"} east: #{@east != nil ? "1" : "0"} west: #{@west != nil ? "1" : "0"}"
  end
...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    irow = 0
    while irow < x_cell_count
      icol = 0
      while icol < y_cell_count
        if irow == 0
          @columns[irow][icol].north = nil
        else
          @columns[irow][icol].north = @columns[irow - 1][icol]
        end
        if irow == x_cell_count - 1
          @columns[irow][icol].south = nil
        else
          @columns[irow][icol].south = @columns[irow + 1][icol]
        end
        if icol == 0
          @columns[irow][icol].west = nil
        else
          @columns[irow][icol].west = @columns[irow][icol - 1]
        end
        if icol == y_cell_count - 1
          @columns[irow][icol].east = nil
        else
          @columns[irow][icol].east = @columns[irow][icol + 1]
        end
        icol += 1
      end
      irow += 1
  ...
  def debug # Debug method - by me
    @columns[0].length.times do |icol|
      @columns.length.times do |irow|
      cell_info = @columns[irow][icol].get_neighbour
        puts "Cell x: #{icol}, y: #{irow} #{cell_info}"
      end
      puts "------------ End of Column -------------"
    end
  end
