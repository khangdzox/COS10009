...
class Food
  ...
  attr_accessor :x, :y, :type, :image, :vel_x, :vel_y, :angle, :x, :y, :score, :smoke, :smoke_x, :smoke_y, :smoked, :start_time
...
def draw_smoke food
  food.smoke.draw(food.x, food.y, ZOrder::FOOD)
end
...
def change_direction food
  begin
    new_x = rand(-2 .. 2)
    new_y = rand(-2 .. 2)
  end until not ((new_x == food.vel_x) and (new_y == food.vel_y))
  food.vel_x = new_x
  food.vel_y = new_y
end
...
class FoodHunterGame < (Example rescue Gosu::Window)
  ...
  def update
    ...
    if rand(200) == 10
      food = @all_food[rand(@all_food.length)]
      food.smoked = true
      food.start_time = Gosu.milliseconds
    end
  ...
  def draw
    ...
    @all_food.each do |food|
      if food.smoked
        draw_smoke food
        if Gosu.milliseconds - food.start_time > 300
          food.smoked = false
